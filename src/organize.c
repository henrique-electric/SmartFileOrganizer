...
int organize(char *path) {
  ...
  while ((file_on_dir = readdir(directory)) != NULL) {
    ...
    if (strcmp(file_on_dir->d_name, ".") == 0 ||
        strcmp(file_on_dir->d_name, "..") == 0 ||
        file_on_dir->d_type == DT_DIR) {
      continue;
    } else {
      memset(file_full_path, 0x0, MAX_FILE_PATH_LEN);
      strncpy(file_info.file_name, file_on_dir->d_name,
              MAX_FILE_NAME_LEN); // copy the current file name to the buffer
