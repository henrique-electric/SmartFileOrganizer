...
void handle_organize(struct program_state *program_struct) {
  ...
  fgets(program_struct->path_buff, USR_PATH_LEN,
        stdin); // get the path to organize
  program_struct->path_buff[strlen(program_struct->path_buff) - 1] =
...
const command_dispatch_table_t command_dispatch_table[] = {
    {handle_organize}, {handle_backup}, {handle_exit}, {handle_status}, // TODO
};
...
int handle_input(struct program_state *program_struct) {
  ...
  flush_std();
  command_dispatch_table[program_struct->user_input - 1].handler(
      program_struct);
