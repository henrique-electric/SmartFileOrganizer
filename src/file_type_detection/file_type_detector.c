...
int get_file_type(const char *file_path, file_info *f_info) {
  ...
  if (file_descriptor == -1) {
    ...
    // Storing into buffer and getting more information about the file
    // descriptor
    unsigned char file_buffer[FILE_BUFFER_SIZE];
    ssize_t file_bytes_read =
        read(file_descriptor, file_buffer, sizeof(file_buffer));
    ...
    if (file_bytes_read == -1) {
      ...
      const char *file_info_details =
          magic_buffer(magic_cookie, file_buffer, file_bytes_read);
      ...
      strncpy(f_info->details, file_info_details,
              strlen(file_info_details)); // Copying details
      f_info->details[strlen(file_info_details) + 1] = '\0';

      char type_buff[8193]; // Buffer for copying details

      strncpy(type_buff, f_info->details,
              strlen(f_info->details)); // Copy details to buffer
      type_buff[strlen(f_info->details) + 1] = '\0';

      char *output =
          strtok(type_buff, ","); // Get only first part (the file type)
      if (output) {
        // Copying type
        const char *short_type = strtok(type_buff, " "); // get the type
        strncpy(f_info->type, short_type,
                sizeof(f_info->type)); // copy the type to the buffer
        f_info->type[strlen(type_buff) + 1] = '\0'; // add a null character
      }

      strncpy(f_info->path, file_path,
              MAX_FILE_NAME_LEN); // Copy the full path to the file
    }
...
int get_file_extension(const char *file_path, file_info *f_info) {
  ...
  if (regexec(&regex, file_path, 2, match, 0) == 0) {
    strncpy(f_info->extension, file_path + match[1].rm_so,
            sizeof(file_path + match[1].rm_so));
...
int type_categories(const char *file_path, file_info *f_info) {
  ...
  const char *sound_formats[] = {
      "mp3", "wav", "flac", "aac",
      "ogg"}; // to do ( adding more extensions/categorizing/grouping etc )
